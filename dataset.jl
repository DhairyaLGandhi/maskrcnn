...
function sample_coco(cid, images, classes;
  ...
  mask, masks = make_masks(segmentation, size(img_data)[1:2], img["file_name"], masks)
  
  # Don't resize
  # img_data = imresize(img_data, 299,299)
  img_data, mask, img_class, rpn_bbox, masks
end

function sample_coco(id::Int, images; base_name = "COCO_train2014_000000"; masks = Dict())
  img = images[id]
  if haskey(img, "file_name")
    img_data = Images.load(joinpath(base, img["file_name"]))
  else
    try
...
function make_masks(segmentation::Array, image_size::Tuple, image_name; masks::Dict = Dict())
  ...
  if !haskey(masks, image_name)
    Drawing(image_size[1:2]..., joinpath(base, "masks/mask_" * image_name))
    background("black")
    sethue("white")
    ...
    masks[image_name] = joinpath(base, "masks/mask_"*image_name)
    i = Images.load(joinpath(base, "masks/mask_"*image_name))
    i, masks
  else
    Images.load(joinpath(base, "masks/mask_"*image_name))
    i, masks
  end
end
