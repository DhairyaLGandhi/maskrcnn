...
function pyramid_roi_align(inputs, pool_size, image_shape)
	...
	# @show roi_level
	roi_level_data = map(x -> isnan(x) || isinf(x) ? 0.0f0 : x, Tracker.data(roi_level)) # remove
...
function proposal_layer(inputs, proposal_count, nms_threshold, anchors, config=Nothing)
	...
	@show "now for nms"
	@show imp[1,:]
	keep = nms2(cpu(imp), nms_threshold)
	@show "keep"
	# @show keep
	# @show boxes[keep,:]
...
function detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config = Nothing)
	...
	negative_roi_bool = cpu(negative_roi_bool) .& no_crowd_bool
	# @info "here"
	...
	if positive_count > 0 && negative_count > 0
		...
		zs = zeros(Int32, negative_count) |> gpu
...
function compute_rpn_class_loss(rpn_match, rpn_class_logits)
	...
	anchor_class = transpose(Flux.onehotbatch(1:maximum(unique(anchor_class)), anchor_class) |> gpu)
...
function compute_mrcnn_class_loss(target_class_ids, pred_class_logits)
	if length(target_class_ids) > 0
		y = Flux.onehotbatch(target_class_ids, 0:80) |> gpu
